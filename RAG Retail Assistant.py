...
def create_enhanced_ui():
    ...
    # Example test set (replace with your real evaluation queries + ground truths)
    predictions = [
        "The price of Product X is $25",
        "Forecasted sales for Product Y are around 120 units next month"
    ]
    references = [
        "Product X costs $25",
        "Sales forecast for Product Y is 120 units next month"
    ]

    if st.button("Run Evaluation"):
        metrics = evaluate_rag(predictions, references)
...
def evaluate_rag(predictions, references):
    ...
    rouge = Rouge()
    scores = [rouge.get_scores(pred, ref)[0] for pred, ref in zip(predictions, references)]
    avg_rouge = {k: np.mean([s[k]['f'] for s in scores]) for k in ['rouge-1','rouge-2','rouge-l']}
    return avg_rouge
